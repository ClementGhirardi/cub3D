...
/*   Updated: 2026/08/19 13:13:57 by clement-ghi      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_config(t_data *data, t_parse *parse)
{
	...
	while (parse->lines[i])
	{
		...
		if (type == LINE_NO || type == LINE_SO || type == LINE_WE
			|| type == LINE_EA || type == LINE_F || type == LINE_C)
		{
			if (parse_config_line(data, parse->lines[i]))
				return (1);
		}
